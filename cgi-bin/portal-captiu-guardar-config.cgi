...
DURADA=$(echo "$QUERY_STRING" | sed -n 's/^.*durada=\([^&]*\).*$/\1/p')
INTERVAL=$(echo "$QUERY_STRING" | sed -n 's/^.*interval=\([^&]*\).*$/\1/p')

RESULTAT=$("$DIR"/"$PROJECTE"/"$DIR_SCRIPTS"/client_srv_cli portal-captiu configurar guardar_config "$DURADA" "$INTERVAL")
