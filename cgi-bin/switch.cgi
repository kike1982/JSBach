...
case $comand in
    ...
    gestion_mac)
        print_header
        echo "<h1>Gestión de MAC</h1><div class='card'><h2>Acciones Globales</h2><a href='/cgi-bin/switch.cgi?comand=apply' class='btn btn-success'>Aplicar Bloqueos</a> <a href='/cgi-bin/switch.cgi?comand=clear' class='btn btn-error'>Borrar Bloqueos</a></div>"
        ...
        while read -r line; do
            ...
            echo "<tr><td><code>$mac</code></td><td><span class='status-badge' style='background:rgba(59,130,246,0.2);color:#60a5fa;'>VLAN $vlan</span></td><td>$status</td><td><a href='/cgi-bin/switch.cgi?comand=del_m&mac=$mac_encoded' class='btn btn-error' style='font-size:0.7rem;padding:0.3rem 0.6rem;'></a></td></tr>"
    ...
    gestion_admin)
        print_header
        echo "<h1>Gestión de Admin (VLAN1)</h1><div class='card'><h2>Acciones Globales</h2><a href='/cgi-bin/switch.cgi?comand=apply_admin' class='btn btn-success'>Aplicar Permisos</a> <a href='/cgi-bin/switch.cgi?comand=clear_admin' class='btn btn-error'>Borrar Reglas</a></div>"
        ...
        for m in $(get_admin_macs); do
            ...
            echo "<div class='mac-item'><span><code>$m</code> $status</span><a href='/cgi-bin/switch.cgi?comand=del_admin_m&mac=$m' class='btn btn-error btn-small' style='padding:0.2rem 0.5rem;'></a></div>"
        done
        echo "</div><script>
    ...
    configurar)
        ...
        while IFS=';' read -r n i u p pr; do
            [ -z "$i" ] && continue
            echo "<tr><td>$n</td><td><code>$i</code></td><td>$u</td><td>${pr^^}</td><td><a href='/cgi-bin/switch.cgi?comand=del_s&ip=$i' class='btn btn-error btn-small' onclick='return confirm(\"¿Seguro?\")'></a></td></tr>"
        done < <(get_switches)
